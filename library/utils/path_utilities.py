    ...
    for material_path in material_search_paths:
        for material_name in material_names:
            ...
            if real_material_path is not None:
                parsed_path = Path(material_path) / material_name
                # Strip leading 'materials/' if it is there
                if parsed_path.parts and parsed_path.parts[0].lower() == 'materials':
                    parsed_path = Path(*parsed_path.parts[1:])
                full_mat_names[material_name] = parsed_path.as_posix()
    for material_name in material_names:
        if material_name not in full_mat_names:
            # Guess using the first search path if available
            if material_search_paths:
                parsed_path = Path(material_search_paths[0]) / material_name
                if parsed_path.parts and parsed_path.parts[0].lower() == 'materials':
                    parsed_path = Path(*parsed_path.parts[1:])
                full_mat_names[material_name] = parsed_path.as_posix()
            else:
