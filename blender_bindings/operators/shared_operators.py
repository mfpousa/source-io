        ...
        for n, obj in enumerate(context.selected_objects):
            ...
            if obj.get("entity_data", None):
                ...
                if model_type == '.vmdl_c':
                    ...
                    skin = custom_prop_data.get('skin', None)
                    if skin is None and "entity" in custom_prop_data:
                        skin = custom_prop_data["entity"].get("skin", None)
                    skin_str = str(skin) if skin is not None else "0"

                    instance_collection = get_collection(prop_path, default_anim, skin_str)
                    if instance_collection and use_collections:
                        collection = bpy.data.collections.get(instance_collection, None)
                        if collection is not None:
                            obj.instance_type = 'COLLECTION'
                    ...
                    try:
                        ...
                        self.report({"WARNING"}, f"Skipped prop {prop_path} because a required file was not found: {e.message}")
                        continue
                    if model_container is None:
                        self.report({"WARNING"},
                                    f"Failed to load MDL file for prop {prop_path}")
                        continue
                    col_name = f"{prop_path.stem}_skin_{skin_str}" if skin_str != "0" else prop_path.stem
                    s1_put_into_collections(model_container, col_name, master_instance_collection, False)

                    if skin_str != "0":
                        for model in model_container.objects:
                            if 'skin_groups' in model and skin_str in model['skin_groups']:
                                skin_materials = model['skin_groups'][skin_str]
                    ...
                    obj["entity_data"]["imported"] = True
                    if use_collections:
                        add_collection(prop_path, model_container.master_collection, default_anim, skin_str)
...
def swap_materials(obj, new_material_name, target_name):
    ...
    target_base = get_base_material_name(target_name)
    print(f'Swapping {target_name} with {new_material_name}')
    for n, obj_mat in enumerate(obj.data.materials):
        if get_base_material_name(obj_mat.name) == target_base:
