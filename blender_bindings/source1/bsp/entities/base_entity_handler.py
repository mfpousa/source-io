...
class BaseEntityHandler(AbstractEntityHandler):
    ...
    def _create_rope_part(self, start_entity: move_rope, start_entity_raw: dict, end_entity: dict):
        ...
        mat = get_or_create_material(stripped_material_name)
    ...
    def handle_infodecal(self, entity: infodecal, entity_raw: dict):
        ...
        for idx, loop in enumerate(mesh_data.loops):
            uv_data[idx].uv = uvs[loop.vertex_index]

        material = get_or_create_material(material_name)
        add_material(material, obj)

        decal_world_pos = Vector(entity.origin) * self.scale
        closest_normal = Vector((0, 0, 1))
        closest_point = decal_world_pos
        min_dist = float('inf')

        for map_obj in self.parent_collection.objects:
            if map_obj.type != 'MESH':
                continue
            if map_obj.name.startswith("infodecal") or "overlay" in map_obj.name:
                continue

            inv_matrix = map_obj.matrix_world.inverted()
            local_pos = inv_matrix @ decal_world_pos
            res, loc, norm, idx = map_obj.closest_point_on_mesh(local_pos)
            if res:
                world_loc = map_obj.matrix_world @ loc
                dist = (world_loc - decal_world_pos).length
                if dist < min_dist:
                    min_dist = dist
                    closest_point = world_loc
                    closest_normal = (map_obj.matrix_world.to_3x3() @ norm).normalized()

        decal_world_pos = closest_point + closest_normal * 0.005
        obj.location = decal_world_pos
        obj.scale = Vector((self.scale, self.scale, self.scale))

        if abs(closest_normal.z) > 0.999:
            align_quat = closest_normal.to_track_quat('-Y', 'X')
        else:
            align_quat = closest_normal.to_track_quat('-Y', 'Z')
