...
def import_bsp(map_path: Path, buffer: Buffer, content_manager: ContentManager, settings: Source1BSPSettings,
    ...
    # Automatically load all placeholders (props) created during the import
    ...
    placeholders = []
    def find_placeholders(col):
        for obj in col.objects:
            if obj.get("entity_data", None) and obj["entity_data"].get("prop_path", None) and not obj["entity_data"].get("imported", False):
                placeholders.append(obj)
        for child in col.children:
            find_placeholders(child)

    find_placeholders(master_collection)
...
def import_materials(bsp: BSPFile, content_manager: ContentManager, settings: Source1BSPSettings, logger: SLogger):
    ...
    def import_source1_materials():
        ...
        for texture_data in texture_data_lump.texture_data:
            ...
            mat = get_or_create_material(tmp)
...
def import_disp(bsp: BSPFile, settings: Source1BSPSettings,
    ...
    for n, disp_info in enumerate(disp_info_lump.infos):
        ...
        material_name = strings_lump.strings[texture_data.name_id] or "NO_NAME"
        material_name = strip_patch_coordinates.sub("", material_name)
        add_material(get_or_create_material(material_name), mesh_obj)
        # mesh_data.validate()
...
def import_overlays(bsp: BSPFile, settings: Source1BSPSettings,
                    master_collection: bpy.types.Collection, logger: SLogger):
    ...
    def gather_meshes(col):
        for o in col.objects:
            if o.type == 'MESH':
                ...
                if not ("decal" in name_lower or "overlay" in name_lower or "trigger" in name_lower or "physics" in name_lower or "tool" in name_lower):
                    mesh_objects.append(o)
        for child in col.children:
            gather_meshes(child)
    gather_meshes(master_collection)

    bsp_textures_info = bsp.get_lump('LUMP_TEXINFO').texture_info
    bsp_textures_data = bsp.get_lump('LUMP_TEXDATA').texture_data
    strings_lump = bsp.get_lump('LUMP_TEXDATA_STRING_TABLE')

    for overlay in overlay_lump.overlays:
        try:
            texture_info = bsp_textures_info[overlay.tex_info]
            ...
            mesh_name = f"overlay_{overlay.id}_MESH"
            obj_name = f"overlay_{overlay.id}"
            ...
            scaled_verts = dst_pos * settings.scale

            from mathutils import Vector
            overlay_normal = Vector(overlay.normal).normalized()
            
            # Cast a ray along the negative normal of the overlay to find the backing wall
            best_translation = None
            min_ray_dist = float('inf')
            
            if mesh_objects:
                overlay_center = Vector(np.mean(scaled_verts, axis=0))
                ray_dir = -overlay_normal
                
                for map_obj in mesh_objects:
                    inv_matrix = map_obj.matrix_world.inverted()
                    local_origin = inv_matrix @ overlay_center
                    local_direction = (inv_matrix.to_3x3() @ ray_dir).normalized()
                    
                    res, loc, norm, idx = map_obj.ray_cast(local_origin, local_direction)
                    if res:
                        world_loc = map_obj.matrix_world @ loc
                        dist = (world_loc - overlay_center).length
                        if dist < min_ray_dist:
                            min_ray_dist = dist
                            best_translation = world_loc - overlay_center

            # Apply translation if a hit was found, otherwise fall back to original positions with bias offset
            # Add render_order to bias to resolve z-fighting between multiple overlapping/stacked overlays
            overlay_bias = 0.005 + (getattr(overlay, "render_order", 0) * 0.001)
            if best_translation is not None:
                scaled_verts = [Vector(v) + best_translation + overlay_normal * overlay_bias for v in scaled_verts]
            else:
                scaled_verts = [Vector(v) + overlay_normal * overlay_bias for v in scaled_verts]

            v0 = Vector(scaled_verts[0])
            v1 = Vector(scaled_verts[1])
            v2 = Vector(scaled_verts[2])
            edge1 = v1 - v0
            edge2 = v2 - v1
            calculated_normal = edge1.cross(edge2).normalized()
            ov_normal = Vector(overlay.normal).normalized()

            if calculated_normal.dot(ov_normal) < 0.0:
                faces = [[0, 3, 2, 1]]
            else:
                faces = [[0, 1, 2, 3]]

            mesh_data.from_pydata(scaled_verts, [], faces)
            ...
            for i, loop in enumerate(mesh_data.loops):
                uv_val = dst_uv[loop.vertex_index]
                uv_data[i].uv = (uv_val[0], 1.0 - uv_val[1])

        except Exception as e:
            logger.exception(f"Failed to import overlay {overlay.id} due to exception:", e)
