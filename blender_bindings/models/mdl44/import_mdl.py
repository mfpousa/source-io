...
def import_model(mdl: MdlV44, vtx: Vtx, vvd: Vvd,
    ...
    for vtx_body_part, body_part in zip(vtx.body_parts, mdl.body_parts):
        for vtx_model, model in zip(vtx_body_part.models, body_part.models):
            ...
            skin_groups = {}
            for n, group in enumerate(mdl.skin_groups):
                resolved_group = []
                for mat_name in group:
                    full_name = full_material_names.get(mat_name, mat_name)
                    mat = get_or_create_material(full_name)
                    resolved_group.append(mat.name)
                skin_groups[str(n)] = resolved_group
            ...
            for mat_id in np.unique(material_indices_array):
                ...
                material = get_or_create_material(full_material_names[mat_name])
...
def import_materials(mdl, use_bvlg=False):
    ...
    for material in mdl.materials:
        ...
        if material_path is None:
            ...
            continue
        mat = get_or_create_material(material_path.as_posix())
